...
function main() {
  ...
  // 정사영 카메라는 절두체 대신 left, right, top, bottom, near, far를 이용하여 '뮥면체'를 정의함.
  // 육면체로 화면을 투사하기 때문에 원근 효과는 없다
  const left = -1;
  const right = 1;
  const top = 1;
  const bottom = -1; // 이렇게 설정해주면 육면체는 기본적으로 너비 2칸(1 - (-1)), 높이 2칸(1 - (-1))로 설정됨.
  // 얘내들은 정확히 육면체 카메라의 left plane, right plane, top plane, bottom plane이 어디에 위치하는가를 정의해 줌.
  // 마치 near plane이 어디에 위치하는가, far plane이 어디에 위치하는가를 정의하는 것과 같음.
  const near = 5;
  const far = 50;
  const camera = new THREE.OrthographicCamera(left, right, top, bottom, near, far);
  camera.zoom = 0.2; // 얘는 perspective camera에서 aspect랑 비슷한, 육면체의 크기를 조절하여 물체를 크게, 또는 작게 보이도록 함.
  ...
  const gui = new GUI();
  gui.add(camera, 'zoom', 0.01, 1, 0.01).listen(); // fov 대신 zoom값을 입력받아 조절할 수 있도록 함.
  // listem()메서드는 dat.GUI가 zoom값에 변화가 생기면 이를 감지해 dat.GUI에 입력폼에 있는 값을 바꿔줌. 왜 쓰냐고?
  // OrbitControls가 마우스 휠 스크롤을 감지해 zoom 속성을 변경하기 때문에, dat.GUI 말고도 zoom값에 변화를 주는 요소가 있는거임.
  // 그래서 dat.GUI외의 요소가 zoom값에 변화를 줄 때, 그 변화를 감지해서 dat.GUI의 입력폼에 있는 값을 바꿔주기 위해 listen()을 사용하는거임. 
  ...
  function animate() {
    ...
    // 첫 번째 화면(기존 화면) 렌더링
    {
      ...
      camera.left = -aspect;
      camera.right = aspect; // 리사이즈로 변경된 scissor area의 비율에 맞게 orthographic 카메라의 비율도 변경해주려면 현재 scissor area의 width / height값인 aspect를 camera.left, right에도 넣어줘야 함. 
